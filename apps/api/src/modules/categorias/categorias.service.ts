...
export class CategoriasService {
  ...
  async create(dto: CreateCategoriaDto) { try { return serializeBigInts(await this.prisma.categorias.create({ data: { id_padre: dto.id_padre ?? null, nombre: dto.nombre.trim(), slug: dto.slug.trim(), descripcion: dto.descripcion ?? null, tipo: dto.tipo?.trim() ?? 'general', orden: dto.orden ?? 0, imagen_url: dto.imagen_url ?? null, activo: dto.activo ?? true } })); } catch (error) { if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') throw new ConflictException('Slug ya existe'); throw error; } }

  async update(id_categoria: number, dto: UpdateCategoriaDto) { const current = await this.prisma.categorias.findUnique({ where: { id_categoria } }); if (!current) throw new NotFoundException('Categoria no encontrada'); return serializeBigInts(await this.prisma.categorias.update({ where: { id_categoria }, data: { id_padre: dto.id_padre ?? undefined, nombre: dto.nombre?.trim(), slug: dto.slug?.trim(), descripcion: dto.descripcion, tipo: dto.tipo?.trim(), orden: dto.orden, imagen_url: dto.imagen_url, activo: dto.activo } })); }
