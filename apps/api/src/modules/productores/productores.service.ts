...
export class ProductoresService {
  ...
  async findByUsuario(id_usuario: string) { const item = await this.prisma.productores.findFirst({ where: { id_usuario, eliminado_en: null }, include: { usuarios: true, regiones: true, lotes: true, tiendas: true } }); if (!item) throw new NotFoundException('Productor no encontrado'); return serializeBigInts(item); }
