...
export class TransportistasService {
  ...
  async createIntegracion(dto: CreateIntegracionEnvioDto) { return serializeBigInts(await this.prisma.integraciones_envio.create({ data: { id_transportista: dto.id_transportista, entorno: dto.entorno?.trim() ?? 'produccion', api_url: dto.api_url ?? null, api_key: dto.api_key ? Buffer.from(dto.api_key) : null, api_secret: dto.api_secret ? Buffer.from(dto.api_secret) : null, credenciales_extra: (dto.credenciales_extra ?? {}) as any, activo: dto.activo ?? true } })); }
  async updateIntegracion(id_integracion: number, dto: UpdateIntegracionEnvioDto) { return serializeBigInts(await this.prisma.integraciones_envio.update({ where: { id_integracion }, data: { id_transportista: dto.id_transportista, entorno: dto.entorno?.trim(), api_url: dto.api_url, api_key: dto.api_key ? Buffer.from(dto.api_key) : undefined, api_secret: dto.api_secret ? Buffer.from(dto.api_secret) : undefined, credenciales_extra: dto.credenciales_extra as any | undefined, activo: dto.activo } })); }
