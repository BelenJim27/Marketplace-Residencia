...
export class UsuariosService {
  ...
  async update(id_usuario: string, dto: UpdateUsuarioDto) {
    ...
    if (dto.google_id && dto.google_id.trim() !== (current.google_id ?? '')) {
  ...
  private async ensureGoogleIdAvailable(google_id: string) {
    const existing = await this.prisma.usuarios.findUnique({ where: { google_id: google_id.trim() } });
