...
function CertificadoPreview({ url }: { url?: string }) {
  if (!url) {
    return (
      <div className="rounded-xl border border-dashed border-[#C5CFB0] bg-[#F4F0E3]/60 px-4 py-8 text-center text-sm text-[#3D6B3F]/50">
  ...
  return (
    <div className="space-y-3">
      {isImage && (
        <div className="overflow-hidden rounded-xl border border-[#C5CFB0] bg-[#F4F0E3]">
      ...
      {!isImage && !isPDF && (
        <div className="flex items-center gap-3 rounded-xl border border-[#C5CFB0] bg-[#F4F0E3]/60 p-4">
          <FileText className="h-8 w-8 shrink-0 text-[#C97A3E]" />
          <p className="text-sm font-medium text-[#1F3A2E]">Documento adjunto</p>
        </div>
      )}
      <a
        href={url}
        ...
        className="inline-flex items-center gap-2 rounded-xl border border-[#C5CFB0] px-4 py-2 text-sm font-medium text-[#1F3A2E] transition hover:bg-[#C5CFB0]/20"
      >
...
export default function SolicitudesProductoresPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pendiente":
        return <span className="inline-flex rounded-full bg-[#C97A3E]/15 text-[#C97A3E] border border-[#C97A3E]/30 px-3 py-1 text-xs font-medium">Pendiente</span>;
      case "aprobado":
        return <span className="inline-flex rounded-full bg-[#A8C26B]/20 text-[#3D6B3F] border border-[#A8C26B]/40 px-3 py-1 text-xs font-medium">Aprobado</span>;
      case "rechazado":
        return <span className="inline-flex rounded-full bg-red-100 text-red-700 px-3 py-1 text-xs font-medium">Rechazado</span>;
      default: return null;
    }
  };
  ...
  return (
    <>
              ...
              {pendientes.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {pendientes.map((solicitud) => (
                    <div key={solicitud.id_productor} className="rounded-2xl border border-[#C5CFB0] bg-white p-4 shadow-[0_2px_8px_rgba(61,107,63,0.06)]">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <h4 className="font-semibold text-[#1F3A2E]">{solicitud.usuarios?.nombre || "Usuario"}</h4>
                      ...
                      {solicitud.categorias && solicitud.categorias.length > 0 && (
                        <div className="mb-4 flex flex-wrap gap-1">
                          {solicitud.categorias.map((cat) => (
                            <span key={cat.id_categoria} className="inline-flex rounded-full bg-[#A8C26B]/20 px-2 py-0.5 text-xs font-medium text-[#3D6B3F]">
                              {cat.nombre}
                            </span>
                          ))}
                        </div>
                    ...
                    <table className="w-full min-w-[640px]">
                      ...
                      <tbody>
                        {paginatedProcesadas.map((solicitud) => (
                          <tr key={solicitud.id_productor} className="border-t border-[#C5CFB0]/30 odd:bg-white even:bg-[#F4F0E3]/40 hover:bg-[#C5CFB0]/20 transition-all duration-200">
                            <td className="px-4 py-3 text-sm font-medium text-[#1F3A2E]">{solicitud.usuarios?.nombre}</td>
                            <td className="px-4 py-3 text-sm text-[#3D6B3F]/70">{solicitud.usuarios?.email}</td>
                            ...
                            <td className="px-4 py-3 text-right">
                              <button
                                onClick={() => setSelectedSolicitud(solicitud)}
                                className="inline-flex items-center gap-1.5 rounded-xl border border-[#C5CFB0] px-3 py-1.5 text-xs font-medium text-[#1F3A2E] transition hover:bg-[#3D6B3F] hover:text-white hover:border-[#3D6B3F]"
                              >
      ...
      {selectedSolicitud && (
        ...
        >
          <div
            className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-[#FDFBF5] border border-[#C5CFB0] shadow-2xl"
          ...
          >
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold text-[#1F3A2E]">
                ...
                <div className="grid grid-cols-2 gap-4 rounded-xl bg-white border border-[#C5CFB0] p-4">
                  <div>
                    ...
                    <p className="text-sm font-medium text-[#1F3A2E]">{selectedSolicitud.usuarios?.nombre}</p>
                  </div>
                  <div>
                    <p className="text-xs text-[#3D6B3F]/60">Email</p>
                    <p className="text-sm font-medium text-[#1F3A2E]">{selectedSolicitud.usuarios?.email}</p>
                  </div>
                  <div>
                  ...
                  {selectedSolicitud.categorias && selectedSolicitud.categorias.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {selectedSolicitud.categorias.map((cat) => (
                        <span key={cat.id_categoria} className="inline-flex rounded-full bg-[#A8C26B]/20 px-3 py-1 text-sm font-medium text-[#3D6B3F]">
                          {cat.nombre}
                        </span>
                      ))}
                    </div>
              ...
              <div>
                ...
                <div className="rounded-xl bg-white border border-[#C5CFB0] p-4 space-y-2">
                  <div>
                    <p className="text-xs text-[#3D6B3F]/60">Fecha de solicitud</p>
                    <p className="text-sm font-medium text-[#1F3A2E]">{new Date(selectedSolicitud.solicitado_en).toLocaleString("es-MX")}</p>
                  </div>
                  {selectedSolicitud.revisado_en && (
                    <div>
                      <p className="text-xs text-[#3D6B3F]/60">Fecha de resolución</p>
                      <p className="text-sm font-medium text-[#1F3A2E]">{new Date(selectedSolicitud.revisado_en).toLocaleString("es-MX")}</p>
                    </div>
                  )}
                  {selectedSolicitud.motivo_rechazo && (
                    <div>
                      <p className="text-xs text-[#3D6B3F]/60">Motivo de rechazo</p>
                      <p className="text-sm font-medium text-red-600">{selectedSolicitud.motivo_rechazo}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {selectedSolicitud.estado === "pendiente" && (
              <div className="border-t border-[#C5CFB0] px-6 py-4 flex gap-4">
                <button
      ...
      {rejectModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ backgroundColor: "rgba(31,58,46,0.45)", backdropFilter: "blur(4px)" }}>
          <div className="w-full max-w-md rounded-2xl bg-[#FDFBF5] border border-[#C5CFB0] shadow-2xl p-6">
            <h2 className="text-xl font-bold text-[#1F3A2E] [font-family:'Playfair_Display',serif] mb-4">Rechazar Solicitud</h2>
            <p className="text-sm text-[#3D6B3F]/70 mb-4">Por favor proporciona un motivo para el rechazo:</p>
            <textarea
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
              rows={4}
              className="w-full rounded-xl border border-[#C5CFB0] bg-white px-4 py-3 text-sm text-[#1F3A2E] focus:border-[#3D6B3F] focus:outline-none focus:ring-1 focus:ring-[#3D6B3F]/20 mb-4"
              placeholder="Motivo del rechazo..."
            />
            <div className="flex gap-3">
              <button onClick={() => { setRejectModalOpen(false); setRejectReason(""); }}
                className="flex-1 rounded-xl border border-[#C5CFB0] px-4 py-2.5 text-sm font-medium text-[#1F3A2E] hover:bg-[#F4F0E3] transition">
                Cancelar
              </button>
      ...
      {approveModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ backgroundColor: "rgba(31,58,46,0.45)", backdropFilter: "blur(4px)" }}>
          <div className="w-full max-w-md rounded-2xl bg-[#FDFBF5] border border-[#C5CFB0] shadow-2xl p-6">
            <h2 className="text-xl font-bold text-[#1F3A2E] [font-family:'Playfair_Display',serif] mb-4">Aprobar Solicitud</h2>
            <p className="text-sm text-[#3D6B3F]/70 mb-4">
