...
export default function LotesView() {
  ...
  const abrirEditar = (lote) => {
    ...
    const complemento = CLASES_CON_COMPLEMENTO.includes(claseBase)
      ? claseRaw.slice(claseBase.length).trim()
      : "";
  ...
  async function guardarLote(e) {
    ...
    const claseCompleta = CLASES_CON_COMPLEMENTO.includes(form.clase) && form.claseComplemento
      ? `${form.clase} ${form.claseComplemento}`
      : form.clase;
    ...
    const payload = {
      ...
      // ── Sitio siempre incluido (crear y editar) ──────────────────────────
      sitio: form.sitio || null,
      datos_api: {
        variedad: form.tipoProducto,
        especie_cientifica: form.especieCientifica,
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6 font-sans">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 rounded-xl bg-white dark:bg-gray-800 p-4 shadow-sm lg:grid-cols-[1.5fr_1fr_1fr_auto]">
          ...
          <button onClick={() => { setSearch(""); setStatus("Todos"); setYear("Todos"); }} className="rounded-lg border border-gray-200 dark:border-gray-600 px-4 py-2 text-sm text-gray-500 hover:bg-gray-50">
            Limpiar
          </button>
        </div>

        {/* ── Tabla: # Lote | Marca | Especie | Grado Alc. | Cantidad | Estado | Acciones ── */}
            ...
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                {["# Lote", "Marca", "Especie", "Grado Alc.", "Cantidad", "Estado", "Acciones"].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500 dark:text-gray-300 whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {filteredLotes.length === 0 ? (
                ...
                filteredLotes.map((item) => (
                  <tr key={item.id_lote} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">{item.lote}</td>
...
function ModalLote({ title, subtitle, onClose, onSubmit, form, setForm, footerActionLabel, loading, categorias }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-800 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {/* Código del Lote */}
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Código del Lote *</label>
            <input
              required
              value={form.lote}
              onChange={(e) => f("lote", e.target.value)}
              className={inputCls}
              placeholder="Ej. CRM-2024-006"
            />
          </div>

          {/* Variedad + Especie Científica */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Variedad / Tipo</label>
              <input
                list="agaves"
                value={form.tipoProducto}
                onChange={(e) => f("tipoProducto", e.target.value)}
                className={inputCls}
                placeholder="Mezcal Espadín"
              />
              <datalist id="agaves">{AGAVE_SUGERIDOS.map(a => <option key={a} value={a} />)}</datalist>
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Especie Científica</label>
              <input
                value={form.especieCientifica}
                onChange={(e) => f("especieCientifica", e.target.value)}
                className={inputCls}
                placeholder="A. angustifolia Haw"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Clase</label>
              <select
                value={form.clase}
                onChange={(e) => {
                  ...
                  setForm((prev) => ({
                    ...prev,
                    clase: nueva,
                    claseComplemento: CLASES_CON_COMPLEMENTO.includes(nueva) ? prev.claseComplemento : "",
                  }));
                }}
                className={inputCls}
              >
          ...
          {/* Complemento de clase — solo si aplica */}
          {necesitaComplemento && (
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">
                {form.clase === "Abocado con" ? "Abocado con..." : "Destilado con..."}
              </label>
              <input
                value={form.claseComplemento}
                onChange={(e) => f("claseComplemento", e.target.value)}
                className={inputCls}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Cantidad (L)</label>
              <input
                type="number" step="0.1" min="0"
                value={form.cantidad}
                onChange={(e) => f("cantidad", e.target.value)}
                className={inputCls}
                placeholder="1500"
              />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Fecha Producción</label>
              <input
                type="date"
                value={form.fechaProduccion}
                onChange={(e) => f("fechaProduccion", e.target.value)}
                className={inputCls}
              />
            </div>
          </div>

          {/* Sitio de Producción — siempre visible (crear y editar) */}
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Sitio de Producción</label>
            <input
              value={form.sitio}
              onChange={(e) => f("sitio", e.target.value)}
              className={inputCls}
              placeholder="Ej. Miahuatlán, Oaxaca"
            />
          </div>

          {/* Descripción */}
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Descripción / Notas</label>
            <textarea
              value={form.descripcion}
              onChange={(e) => f("descripcion", e.target.value)}
              className={`${inputCls} h-20 resize-none`}
              placeholder="Notas de producción, sabor, proceso..."
            />
          </div>

          {/* Acciones */}
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700">
              Cancelar
            </button>
            <button
              disabled={loading}
...
function DetalleLoteModal({ lote, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-white p-6 dark:bg-gray-800 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <h2 className="mb-4 text-xl font-bold dark:text-white">Detalles: {lote.lote}</h2>
        <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
          {rows.map(([label, value]) => (
            <div key={label}>
              <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">{label}</p>
              <p className="mt-0.5 font-medium text-gray-800 dark:text-gray-100">{value || "—"}</p>
            </div>
          ))}
          <div className="col-span-2 mt-2 border-t pt-4 dark:border-gray-700">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Descripción</p>
        ...
        <button
          onClick={onClose}
          className="mt-6 w-full rounded-lg bg-gray-100 py-2 text-sm font-bold hover:bg-gray-200 dark:bg-gray-700 dark:text-white"
        >
          Cerrar
        </button>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 text-center dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="text-lg font-bold dark:text-white">¿Eliminar lote?</h3>
        <p className="mt-1 text-sm text-gray-500">
          Esta acción borrará el lote <b>{lote.lote}</b> de forma permanente.
        </p>
        <div className="mt-6 flex gap-3">
          <button onClick={onClose} className="flex-1 rounded-lg border py-2 text-sm font-medium dark:border-gray-600 dark:text-gray-300">
            Cancelar
          </button>
          <button
            onClick={onConfirm}
